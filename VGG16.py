...
CONFIG = {
    ...
    "image_size":      224,
    "batch_size":      4, # SMALLER FOR VGG16
    "num_epochs":      20,
...
def build_model(num_classes, freeze_backbone=True):
    ...
    # Freeze backbone initially (features only)
    if freeze_backbone:
        for param in model.features.parameters():
            param.requires_grad = False

    # Replace classifier (VGG uses model.classifier)
...
def train_model(model, train_loader, val_loader):
    ...
    for epoch in range(1, CONFIG["num_epochs"] + 1):
        ...
        if epoch == CONFIG["fine_tune_epoch"]:
            print("\n>>> Unfreezing VGG16 feature extractor...\n")
            for param in model.features.parameters():
    ...
    print(f"\nBest model saved to: best_model_best_model_vgg16.pth")
    return history
...
def plot_results(history, all_labels, all_preds, class_names):
    ...
    fig.suptitle("VGG16 — Training Results", fontsize=13,
                 fontweight="bold")
    ...
    plt.tight_layout()
    plt.savefig("training_curves_vgg16.png", dpi=150)
    plt.show()
    ...
    plt.show()
    print("Saved: training_curves_vgg16.png, confusion_matrix_vgg16.png")
...
def predict_image(image_path, model, class_names, device):
    ...
    fig.suptitle(f"VGG16 — Gonadal Stage Prediction\n"
    ...
    axes[1].invert_yaxis()
    plt.tight_layout()
    out_path = os.path.splitext(image_path)[0] + "_vgg16_prediction.png"
