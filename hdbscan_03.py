...
model = hdbscan.HDBSCAN(min_cluster_size=2, min_samples=1, cluster_selection_method='eom')
...
if len(np.unique(labels[valid])) > 1:
    ...
    print(f"HDBSCAN Silhouette: {sil:.4f}")
else:
    print("HDBSCAN: Not enough clusters for silhouette.")

clusters = sorted(list(set(labels) - {-1}))

for cid in clusters:
    path = f"hdbscan_clusters/cluster_{cid}"
...
# Save noise
os.makedirs("hdbscan_clusters/noise", exist_ok=True)
for img, lbl, fname in zip(images, labels, names):
    if lbl == -1:
        # save noise similarly
        if os.sep in fname or ("/" in fname) or ("\\" in fname):
            comps = [c for c in re.split(r"[\\/]+", fname) if c]
        else:
            comps = [c for c in re.split(r"[_\- ]+", fname) if c]
        ...
        save_path = os.path.join("hdbscan_clusters/noise", f"{save_name}.jpg")
        try:
            bgr = cv2.cvtColor(img, cv2.COLOR_RGB2BGR)
        except Exception:
            bgr = img
