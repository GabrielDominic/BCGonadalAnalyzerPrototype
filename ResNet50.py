...
def build_model(num_classes, freeze_backbone=True):
    """
    Load pretrained ResNet-50 and replace the fc head for 4-class output.

    Fine-tuning phases:
      Phase 1 (epochs 1 → fine_tune_epoch): fc head only
      Phase 2 (fine_tune_epoch onwards): layer3 + layer4 + fc unfrozen
    """
    ...
    print(f"  Trainable    : {trainable:,} (fc head — backbone frozen)\n")
...
def train_model(model, train_loader, val_loader):
    ...
    print("=" * 55)
    print("STEP 4: Training ResNet-50")
    print("=" * 55)
    ...
    for epoch in range(1, CONFIG["num_epochs"] + 1):
        ...
        if epoch == CONFIG["fine_tune_epoch"]:
            print("\n>>> Unfreezing layer3, layer4, fc for fine-tuning...\n")
    ...
    print(f"\nBest model saved to: best_model_resnet50.pth")
    return history
...
def evaluate_on_test(model, test_loader, class_names):
    ...
    print("\n" + "=" * 55)
    print("TEST SET RESULTS — ResNet-50")
    print("=" * 55)
...
def plot_results(history, all_labels, all_preds, class_names):
    ...
    fig.suptitle("ResNet-50 — Training Results", fontsize=13,
                 fontweight="bold")
    ...
    plt.show()
    print("Saved: training_curves_resnet50.png, confusion_matrix_resnet50.png")
...
def predict_image(image_path, model, class_names, device):
    ...
    fig.suptitle(f"ResNet-50 — Gonadal Stage Prediction\n"
    ...
    axes[1].invert_yaxis()
    plt.tight_layout()
    out_path = os.path.splitext(image_path)[0] + "_resnet50_prediction.png"
